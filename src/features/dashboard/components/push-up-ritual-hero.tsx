...
export function PushUpRitualHero() {
  return (
    <section
      ...
      px-6 pb-16 pt-[clamp(9rem,18vh,12rem)] isolate"
      id="challenge"
    ...
    >
      <img
        ...
        decoding="async"
      />
      <div className="hero-content relative z-2 w-full max-w-215">
        <p className="mb-4 text-xs font-bold tracking-[0.16em] text-pupz-accent uppercase">
          Daily push-up challenge
        </p>
        <h1
          className="max-w-225 font-pupz-display text-[clamp(3.5rem,8.4vw,8rem)] leading-[0.86]
          font-extrabold tracking-[-0.08em] text-pupz-ink uppercase text-balance"
          id="hero-title"
        >
          Build your
          <br />
        ...
        <p className="my-6 mb-8 max-w-136 text-[clamp(1rem,1.4vw,1.125rem)] leading-normal text-pupz-ink-muted">
        ...
        <Link
          className={cn(
            ...
            "min-h-12 gap-4 rounded-[10px] bg-pupz-accent px-6 text-xs font-bold tracking-[0.08em]",
            "text-pupz-accent-ink uppercase transition-[background-color,transform,box-shadow]",
            "hover:bg-pupz-accent/90 hover:-translate-y-0.5 active:translate-y-px active:scale-[0.985]",
            "hover:shadow-[0_12px_30px_color-mix(in_srgb,var(--color-accent)_18%,transparent)]",
          )}
          to="/exercise/push-up"
        >
          Start today
      ...
      <Card
        className="metric-panel absolute right-6 bottom-16 z-3 w-[min(250px,calc(100%-48px))] overflow-hidden
        rounded-[18px] bg-pupz-hero-panel/80 p-0 text-pupz-ink backdrop-blur-lg"
      ...
      <a
        className="absolute right-6 bottom-6 z-2 inline-flex items-center gap-2 text-[0.6875rem]
        font-semibold tracking-widest text-pupz-ink-dim uppercase transition-colors
        hover:text-pupz-ink max-[800px]:hidden"
        href="#progress"
      >
        Scroll to progress
